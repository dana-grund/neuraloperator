...
from neuralop.losses import compute_deterministic_scores, print_scores, H1Loss, LpLoss, singleEnsemble_baseline_mmd
from neuralop.models import TFNO
import time

"""
Script for computing the MMD for the baseline dataset.
Tries to compute the MMD of one huge ensemble containing the whole dataset.
...
set_size = 10000
mmd = singleEnsemble_baseline_mmd(model, test_db, data_processor, set_size)

endCalc = time.time()

print(f"Single Ensemble Baseline mmd ({set_size}): {mmd}")
print(f"Set up time: {endSetup-startSetup}, Calculation time: {endCalc-startCalc}")

f = open(f"singleEnsemble_baselineMmdFNO{set_size}.txt", "x")
f.write(f"{mmd}")
f.close()