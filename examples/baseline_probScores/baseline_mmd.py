...
from neuralop.losses import compute_deterministic_scores, print_scores, H1Loss, LpLoss, baseline_mmd
from neuralop.models import TFNO
import time

"""
Script for computing the MMD for the baseline dataset.
Computes the MMD as the average over the 100 MMD values of the ensembles.
"""
...
set_size = 10000
mmd = baseline_mmd(model, test_db, data_processor, set_size)

endCalc = time.time()

print(f"Baseline mmd ({set_size}): {mmd}")
print(f"Set up time: {endSetup-startSetup}, Calculation time: {endCalc-startCalc}")

f = open(f"baselineMmdFNO{set_size}_correctedEns_new.txt", "x")
f.write(f"{mmd}")
f.close()