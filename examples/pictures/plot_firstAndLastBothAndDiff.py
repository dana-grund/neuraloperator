...
"""
Script for ploting u field at timestep 0 and prediction and ground truth at the final timestep and their difference.
...
caxList.append(axs[0].imshow(field[0], extent=[0, 127, 0, 127]))
#divider = make_axes_locatable(axs[row,col])
#cax = divider.append_axes("right", size="6%", pad=0.06)
fig.colorbar(caxList[-1], ax=axs[0], fraction=0.046, pad=0.04, label='u')
...
#divider = make_axes_locatable(axs[row,col])
#cax = divider.append_axes("right", size="6%", pad=0.06)
fig.colorbar(caxList[-1], ax=axs[1], fraction=0.046, pad=0.04, label='u')
...
#divider = make_axes_locatable(axs[row,col])
#cax = divider.append_axes("right", size="6%", pad=0.06)
fig.colorbar(caxList[-1], ax=axs[2], fraction=0.046, pad=0.04, label='u')
...
#divider = make_axes_locatable(axs[row,col])
#cax = divider.append_axes("right", size="6%", pad=0.06)
fig.colorbar(caxList[-1], ax=axs[3], fraction=0.046, pad=0.04, label='Difference in u')
...
fig.suptitle("Baseline Label vs Prediction - u", fontsize=16, x=0.43, y=0.72)  # Raise the title
...
plt.savefig(f"firstLastBothDiffRegular1.png")
...
# Prediction
field = labelVelsIn[0, 0, :, :] - velocitiesIn[0, 0, :, :]
caxList.append(axs[3].imshow(field, extent=[0, 127, 0, 127]))
#divider = make_axes_locatable(axs[row,col])
#cax = divider.append_axes("right", size="6%", pad=0.06)
fig.colorbar(caxList[-1], ax=axs[3], fraction=0.046, pad=0.04, label='Difference in u')
...
#fig.subplots_adjust(hspace=0.005, wspace=0.3, right=0.85)  # Decrease hspace to reduce vertical space
fig.subplots_adjust(hspace=0.3, wspace=0.4, top=0.9)
fig.suptitle("Ensemble Label vs Prediction - u", fontsize=16, x=0.43, y=0.72)  # Raise the title
...
plt.savefig(f"firstLastBothDiffEnsemble1.png")