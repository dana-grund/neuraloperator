...
Sample from the ensemble dataset.
"""
...
print(f"Num. batches: {num_batches}")
num_batches = int(num_batches) + 1
velocitiesTest, labelVelsTest = forwardPass(model, ensemble_loaders[0], num_batches, data_processor)
...
fig.suptitle("Ensemble Data - Ground truth vs. Prediction", fontsize=16, x=0.43, y=0.91)  # Raise the title
...
plt.savefig(f"LastBothDiffVUEnsemble1.png")
