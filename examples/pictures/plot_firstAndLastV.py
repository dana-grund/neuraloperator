...
"""
Script for plotting the ground truth v field of a samples of the baseline dataset.
...
caxList.append(axs[0].imshow(field[1].T, extent=[0, 127, 0, 127]))
...
field = Fields[10]
caxList.append(axs[1].imshow(field[1].T, extent=[0, 127, 0, 127]))
#divider = make_axes_locatable(axs[row,col])
#cax = divider.append_axes("right", size="6%", pad=0.06)
...
fig.suptitle("Label v", fontsize=16, x=0.43, y=0.75)  # Raise the title
...
plt.savefig(f"firstAndLast_regularV{size}.png")
...
plt.savefig(f"firstAndLast_ensembleV{size}.png")
