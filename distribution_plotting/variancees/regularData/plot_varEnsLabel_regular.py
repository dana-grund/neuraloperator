...
import osSince here the target is the baseline dataset, the ensembles are packs of 100 samples taken in order of the dataset.
...
"""
Script for plotting ground truth u variance of the baseline dataset.
...
for i in range(10):
    ...
    col = i % 2
    row = i // 2
    caxList.append(axs[row,col].imshow(sigs_in_lab, extent=[0, 127, 0, 127]))
...
fig.suptitle("Label Variances in u - Regular Data", fontsize=16, x=0.43, y=0.92)  # Raise the title
...
plt.savefig("ensPlots/VarEnsLabelU_regular.png")
