...
"""
Script for plotting the relative differences (ground truth - prediction) in mean u over ensemble 1 and 2 of the baseline dataset.
...
for i in range(2):
    ...
    caxList.append(axs[i].imshow(sigs_in_err, extent=[0, 127, 0, 127]))
    #divider = make_axes_locatable(axs[row,col])
    #cax = divider.append_axes("right", size="6%", pad=0.06)
    fig.colorbar(caxList[-1], ax=axs[i], fraction=0.046, pad=0.04)
    if i == 0:
...
fig.suptitle("Relative Mean Differences in v - Baseline Data", fontsize=16, x=0.43, y=0.92)  # Raise the title
...
plt.savefig("ensPlots/MeanEnsRelDiffV_regular1And2.png")
