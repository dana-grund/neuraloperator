...
"""
Script for plotting the relative errors in variance over an ensembles at each point for u.
...
for i in range(10):
    ...
    rel_sig_errors = np.abs(sigs_in_lab - sigs_in_pred) / sigs_in_lab

    col = i % 2
    row = i // 2
    caxList.append(axs[row,col].imshow(rel_sig_errors, extent=[0, 127, 0, 127]))
...
fig.suptitle("Relative Variance Errors in u", fontsize=16, x=0.43, y=0.92)  # Raise the title
...
plt.savefig("meanRelativeVarDiffsEnsU.png")