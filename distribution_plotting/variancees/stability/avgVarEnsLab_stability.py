...
"""
Script for computing the average variances of u and v over the members of an ensemble 
of the ground truth velocities.
Here the ensembles are taken from the stability dataset.
"""
...
velocitiesIn, labelVelsIn = forwardPass(model, ensemble_loaders[1], num_batches, data_processor)
...
for i in range(10):
    ...
    #u in
    predIn = labelVelsIn[i*100:(i+1)*100, 0, :, :]

    sigs_in_pred = np.var(predIn, axis=0)

    ensAvgU = np.mean(sigs_in_pred)
    AvgU += (1./10.)*ensAvgU


    #v in
    predIn = labelVelsIn[i*100:(i+1)*100, 1, :, :]

    sigs_in_pred = np.var(predIn, axis=0)

    ensAvgV = np.mean(sigs_in_pred)
    AvgV += (1./10.)*ensAvgV
