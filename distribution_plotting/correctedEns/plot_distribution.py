...
Refer to other plot_distribution.py in parent directory.
"""
...
for i in range(0, 5):
    ...
    axs[i].set_title('Ensemble Data - '+vel+f' Distribution, Locatin ({x_idx}, {y_idx}), Ensemble {i+1}')
...
plt.tight_layout()
if not os.path.exists("ensembleData/" + f"{x_idx}_{y_idx}"):
    os.mkdir("ensembleData/" + f"{x_idx}_{y_idx}")
plt.savefig("ensembleData/" + f"{x_idx}_{y_idx}/" + "ensemble_distributions_" + vel + f"_1to5_x={x_idx}_y={y_idx}.png")
...
for i in range(5, 10):
    ...
    axs[i-5].set_title('Ensemble Data - '+vel+f' Distribution, Locatin ({x_idx}, {y_idx}), Ensemble {i+1}')
...
plt.tight_layout()
plt.savefig("ensembleData/" + f"{x_idx}_{y_idx}/" + "ensemble_distributions_" + vel + f"_6to10_x={x_idx}_y={y_idx}.png")
